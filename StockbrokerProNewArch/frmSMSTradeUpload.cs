...
{
    public partial class frmSMSTradeUpload : Form
    {

        private DataTable TradeData;
        private string FileName;
        
        public frmSMSTradeUpload()
        {
            InitializeComponent();

            //Default Value Loaded
            _isOldTradedFileLoaded = true;
        }
        private bool _isOldTradedFileLoaded;

        public bool IsOldTradedFileLoaded
        {
            get { return _isOldTradedFileLoaded; }
            set { _isOldTradedFileLoaded = value; }
        }
        ...
        private void btnStartImport_Click(object sender, EventArgs e)
        {
            if (ValidateFeild())
            {
                try
                {
                    ...
                    this.Hide();
                    frmSMSConfirmationProcess smsconfirmationProcess = new frmSMSConfirmationProcess(MenuNameList.SMS_Trade_Confirmation);
                    smsconfirmationProcess.ShowDialog();
                }
                catch (Exception exc)
                {
        ...
        //    try
        //    {
        //        if (!_isOldTradedFileLoaded)
        //        {
        //            tradeDataTable = ProcessTradeFile(txtFileLocation.Text);
        //            PayInTradeBAL tradeBal = new PayInTradeBAL();
        //            tradeBal.TruncateTradeTemp();
        //            tradeBal.SaveProcessedTradeTemp(tradeDataTable, "SBP_Trade_Temp");
        //            SMSDataEditBAL smsDataEditBal = new SMSDataEditBAL();
        //            smsDataEditBal.GenerateSmsConfTempTable();
        //        }
        //        else
        //        {
        ...
        //            SMSDataEditBAL smsDataEditBal = new SMSDataEditBAL();
        //            smsDataEditBal.GenerateSmsConfOldTempTable();
        //        }
