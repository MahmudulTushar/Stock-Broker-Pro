...
{
    public partial class frmConversionWithdrawalUpdate : Form
    {
        public frmConversionWithdrawalUpdate()
        {
            InitializeComponent();
        }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Want to Run Withdraw Data Update Process?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
            {
                ShareDWBO_Conversion_Collection collectionBO = new ShareDWBO_Conversion_Collection();
                ShareDWBAL BAL = new ShareDWBAL();
                ShareDWBO_Conversion BO = new ShareDWBO_Conversion();
                ...
                foreach (ShareDWBO_Conversion DW_BO in collectionBO)
                {
                    try
                    {
                        BAL.UpdateRecord_Withdraw(DW_BO);
                    }
                    catch (Exception ex)
                    {
                ...
                MessageBox.Show("Share Withdraw Data Update Process Completed....", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
}
