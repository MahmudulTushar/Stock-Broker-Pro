...
namespace StockbrokerProNewArch
{
    public partial class EFT_Issue : Form
    {
        ...
        public EFT_Issue()
        {
            ...
            cmbBankSelection.SelectedIndex = 0;
        ...
        private void Export_xls(string FileName)
        {
            ...
            var dataList = objBal.GetExport_ExcelDocByFileNo_Export(txtFileNo.Text.Trim(),Indication_CompanyBankInformation.CityBank_TradeAccount_AccountNo,cmbBankSelection.Text);
            ...
            foreach (var obj in dataList)
            {
                ...
                currentSheet.Cells[count_Row, 8] = Convert.ToString(objBal.GetCustomerName(obj.Cust_Code, cmbBankSelection.Text)).Trim();
                count_Row++;
            }
        ...
        private void InsertEft_Issue(List<Payment_PostingBO> objList)
        {
            ...
            foreach (var obj in objList)
            {
                ...
                if (Indication_PaymentTransaction.ReqType_EftIssue_TradeAccount == obj.ReqType)
                {
        ...
        private void EFT_Issue_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                var objList = Eft.GetAllApprovedRequest_BOList(cmbBankSelection.Text);
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cmbBankSelection.Text == "Others Bank")
                {
                    Export_xls_Encrypt(txtFilePath.Text + "\\" + txtFileNo.Text.Trim());
                }
                objBal.UpdateExportFlag_EFT_FileInfo(txtFileNo.Text.Trim());
                
                //Inserting File Contents
                var dataList = objBal.GetExport_ExcelDocByFileNo_Export(txtFileNo.Text, Indication_CompanyBankInformation.CityBank_TradeAccount_AccountNo,cmbBankSelection.Text);
        ...
        private void Report_Preview(string report)
        {
            if (report == Reports_Type.Summary_Reports_Name)
            {
                ...
                dtBEFTN = EFT_IssueBAL.GetBEFTN_ReportData(txtFileNo.Text, Indication_CompanyBankInformation.CityBank_TradeAccount_AccountNo,cmbBankSelection.Text); 
            ...
            {
                EFT_IssueBAL EFT_IssueBAL = new EFT_IssueBAL();
                crEFT_Issue_CityBank crEFT_Issue_cityBank = new crEFT_Issue_CityBank();
                crEFT_Issue crEFT_Issue = new crEFT_Issue();
                frmReportViewer ReportViewer = new frmReportViewer();
                var dtEFT_Issue = EFT_IssueBAL.GetExport_ExcelDocByFileNo_Report(txtFileNo.Text.Trim(), Indication_CompanyBankInformation.CityBank_TradeAccount_AccountNo,cmbBankSelection.Text);
                if (cmbBankSelection.Text == "THE CITY BANK LTD.")
                {
                    crEFT_Issue_cityBank.SetDataSource(dtEFT_Issue);
                    ReportViewer.crvReportViewer.DisplayGroupTree = false;
                    ReportViewer.crvReportViewer.ReportSource = crEFT_Issue_cityBank;
                    ReportViewer.crvReportViewer.ReportSource = crEFT_Issue_cityBank;
                    ReportViewer.crvReportViewer.ShowExportButton = false;
                }
                else
        ...
        private void cmbBankSelection_SelectedIndexChanged(object sender, EventArgs e)
        {
            EFT_Issue_Load(sender, e);
        }
        private void Encrypt()
        {
            this.openFileDialog1.Multiselect = false;
            this.openFileDialog1.InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            this.openFileDialog1.Filter = "Microsoft Excel Files 97-2003 (*.xls)|*.xls|Microsoft Excel Files(*.xlsx)|*.xlsx|(*.txt)|*.txt";
            this.openFileDialog1.FileName = "";
            this.openFileDialog1.ShowDialog();
            if (this.openFileDialog1.FileName != "")
            {
                string fileName = this.openFileDialog1.FileName;
                ValidateExelField field = new ValidateExelField();
                string str2 = string.Empty;
                str2 = field.ValidateExcelForEFT(fileName).Trim();
                if (str2.Equals(string.Empty))
                {
                    this.ZipThis(fileName);
                    //this.txtEFTError.Text = string.Empty;
                    MessageBox.Show("Encrypted");
                }
                else
                {
                    //this.txtEFTError.Text = str2;
                    MessageBox.Show("Encryption failed");
                }
            }
        }
        private void ZipThis(string sourcefile)
        {
            ...
            SaveFileDialog dialog = new SaveFileDialog
            {
                ...
                FileName = fileName,
                Filter = ".exl|All Files(*.*)",
                FilterIndex = 1
            };
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                fileName = dialog.FileName + ".exl";
            }
            using (ZipFile file = new ZipFile())
            {
                file.Encryption = EncryptionAlgorithm.WinZipAes256;
                file.Password = "e#hTgz//23:0@5wPE+6H/TT.0+Am=Y:^I0S68#Ih!F~#uAxPQ^AJ^:9i4B!TVP4y";
                file.AddFile(sourcefile, "");
                try
                {
                    file.Save(fileName);
                }
        ...
        private void Export_xls_Encrypt(string FileName)
        {
            EFT_IssueBAL objBal = new EFT_IssueBAL();
            //int dataSartingRowIndex = 1;
            int dataSartingRowIndex = 1;
            ...
            workbook.Author = "K-Secuirites Ltd.";
            workbook.SaveAs(FileName + "E", Microsoft.Office.Interop.Excel.XlFileFormat.xlOpenXMLWorkbook, "", missValue,
             false, false, Microsoft.Office.Interop.Excel.XlSaveAsAccessMode.xlNoChange,
             missValue, missValue, missValue, missValue, missValue);
            workbook.Close(true, missValue, missValue);
            excelApp.Quit();
