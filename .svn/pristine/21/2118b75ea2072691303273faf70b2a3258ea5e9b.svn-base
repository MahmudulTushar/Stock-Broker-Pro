...
{
    public class StockSplitBAL
    {
        private DbConnection _dbConnection;

        public StockSplitBAL()
        {
        ...
        {
            DataTable dataTable;
            string queryString = "SELECT DISTINCT(ISIN_No) From SBP_17DP70UX WHERE ISIN_No NOT IN (SELECT ISIN_No From SBP_Company) AND CA_Type='Stock Split'";
            try
            {
                _dbConnection.ConnectDatabase();
        ...
        {
            DataTable dataTable;
            string queryString = "SELECT DISTINCT(BO_ID) From SBP_17DP70UX WHERE BO_ID NOT IN (SELECT BO_ID From SBP_Customers) AND CA_Type ='Stock Split'";
            try
            {
                _dbConnection.ConnectDatabase();
                dataTable = _dbConnection.ExecuteQuery(queryString);
        ...
        public void SaveIntoShareDW()
        {
            ...
            + "SELECT dbo.GetCustCodeFromBO(BO_ID),dbo.GetCompShortCodeFromISINNo(ISIN_No),Total_Entitlement,0,Total_Entitlement,'CDBL',0,(dbo.GetBuyAvg(dbo.GetCustCodeFromBO(BO_ID),Effective_Date,dbo.GetCompShortCodeFromISINNo(ISIN_No))/Issue_Price)*Total_Entitlement,Effective_Date,Effective_Date,Record_Date,(CASE WHEN(DR_CR_Flag='CREDIT') THEN 'Deposit' ELSE 'Withdraw' END),CA_Type,Getdate(),'" + GlobalVariableBO._userName + "' From SBP_17DP70UX WHERE CA_Type='Stock Split'";
            ...
            queryStringInsHistory = "INSERT INTO  SBP_17DP70UX_History(ISIN_No,ISIN_Short_Name,Sequence_No,CA_Type,Record_Date,Effective_Date,Cash_Fraction_Amount,Payment_Date,Benefit_ISIN,Benefit_ISIN_Short_Name,BO_ID,BO_Short_Name,BO_Holding,DR_CR_Flag,Total_Entitlement,Parent_Ration,Benefit_Ration,un1,un2,un3,Issue_Price)" +
                "SELECT ISIN_No,ISIN_Short_Name,Sequence_No,CA_Type,Record_Date,Effective_Date,Cash_Fraction_Amount,Payment_Date,Benefit_ISIN,Benefit_ISIN_Short_Name,BO_ID,BO_Short_Name,BO_Holding,DR_CR_Flag,Total_Entitlement,Parent_Ration,Benefit_Ration,un1,un2,un3,Issue_Price From SBP_17DP70UX WHERE CA_Type='Stock Split'";
            CommonBAL comBAL = new CommonBAL();
            long slNo = comBAL.GenerateID("SBP_Upload_History", "Upload_ID");
            queryStringUploadHistory = "INSERT INTO SBP_Upload_History(Upload_ID,File_Name,Upload_Date,Entry_By) SELECT TOP 1 " + slNo + ",'17DP70UX_StockSplit',Effective_Date,'" + GlobalVariableBO._userName + "' FROM SBP_17DP70UX WHERE CA_Type='Stock Split'";
        ...
        public DataTable GetGridIssueAmount()
        {
            ...
            DataTable dataTable;
            string queryString = "SELECT DISTINCT(ISIN_Short_Name) AS 'Company Name',CA_Type AS 'Deposit Type',10 AS 'Split Value' From SBP_17DP70UX WHERE CA_Type='Stock Split'";
            try
            {
                _dbConnection.ConnectDatabase();
                dataTable = _dbConnection.ExecuteQuery(queryString);
        ...
        {
            DataTable dataTable;
            string queryString = "SELECT ISIN_Short_Name as 'Company Short Name',BO_ID as 'BO ID',Total_Entitlement as 'Share Quantity',CA_Type as 'Deposit Type',Record_Date as 'Record Date',Effective_Date as 'Effective Date' From SBP_17DP70UX WHERE CA_Type ='Stock Split'";
            try
            {
                _dbConnection.ConnectDatabase();
                dataTable = _dbConnection.ExecuteQuery(queryString);
        ...
        public void UpdateIssueAmount(string companyName, float issuePrice)
        {
            ...
            queryStringUpdateTemp = "UPDATE  SBP_17DP70UX SET Issue_Price=" + issuePrice + " WHERE ISIN_Short_Name='" + companyName + "' AND CA_Type ='Stock Split'";
            try
            {
                _dbConnection.ConnectDatabase();
                _dbConnection.StartTransaction();
